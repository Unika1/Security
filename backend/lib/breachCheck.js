import crypto from "crypto";
  ...
  //   4. Check hash ending against that list locally.
  // If the check cannot run (for example no internet), password is allowed so
...
export async function isPasswordBreached(password) {
  try {
    ...
    // Each line looks like "SUFFIX:count". If suffix is there, it is breached.
